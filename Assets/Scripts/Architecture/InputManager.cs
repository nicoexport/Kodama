...
namespace Kodama.Architecture {
    public class InputManager : MonoBehaviour {
        ...
        private void OnEnable() => _playerInput.onActionTriggered += OnActionTriggered;

        private void OnDisable() => _playerInput.onActionTriggered -= OnActionTriggered;
        ...
        private void OnActionTriggered(InputAction.CallbackContext obj) {
            ...
            if (nextInputDevice == CurrentInputDevice) {
                return;
            }
        ...
        private void FillDeviceMapDict() {
            ...
            for (int index = 0; index < _deviceMap.DeviceNames.Count; index++) {
                string name = _deviceMap.DeviceNames[index];
        ...
        public static void DisableInput() => playerInputActions.Disable();

        public static GeneralDeviceType? GetGeneralDeviceTypeByName(string deviceName) {
            bool success = _deviceMapDictionary.TryGetValue(deviceName, out var type);
            if (success) {
                return type;
            }
        ...
        public static float GetHorizontalMovementValue() {
            float input = playerInputActions.Player.HorizontalMovement.ReadValue<float>();
        ...
        public static float GetVerticalMovementValue() {
            float input = playerInputActions.Player.VerticalMovement.ReadValue<float>();
            if (Mathf.Abs(input) < movementDeadZoneMin) {
                return 0f;
            }
