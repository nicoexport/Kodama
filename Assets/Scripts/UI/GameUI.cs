...
namespace UI {
    public class GameUI : Resettable {
        ...
        public override void OnLevelReset() => EnableGameUI();

        [ContextMenu("DisableGameUI")]
        private void DisableGameUI(LevelData levelData) => levelTimerUI.SetActive(false);

        [ContextMenu("EnableGameUI")]
        private void EnableGameUI() => levelTimerUI.SetActive(true);
